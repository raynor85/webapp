...
public class SyncBackSeRemoteRetriever implements RemoteRetriever {
	...
	public String retrieveWin32UrlEn(Document doc) {
		try {
			return RemoteServiceImpl.retrieveHtmlDocumentAgentMozilla(ParsingUtils.buildUrl(ROOT_DOWNLOAD_WEBSITE, doc.select("a:contains(Download SyncBackSE)").attr("href"))).select("a:contains(Download)[href*=.exe]").attr("href");
		} catch (IOException e) {
			return null;
		}
	}
