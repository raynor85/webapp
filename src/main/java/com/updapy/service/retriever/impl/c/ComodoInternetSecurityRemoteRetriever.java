...
public class ComodoInternetSecurityRemoteRetriever implements RemoteRetriever {
	...
	public String retrieveWin32UrlEn(Document doc) {
		try {
			return StringUtils.removePattern(RemoteServiceImpl.retrieveHtmlDocumentAgent32(DOWNLOAD_WEBSITE).select("img[title*=Download Free Internet Security Software]").first().parent().select("a[href*=exe]").attr("href"), "\\?.*");
		} catch (IOException e) {
			return null;
		}
	}
