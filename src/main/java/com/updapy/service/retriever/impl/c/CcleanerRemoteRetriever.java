...
public class CcleanerRemoteRetriever implements RemoteRetriever {
	...
	public String retrieveWin32UrlEn(Document doc) {
		try {
			return RemoteServiceImpl.retrieveHtmlDocumentAgent32(ParsingUtils.buildUrl(ROOT_DOWNLOAD_WEBSITE, doc.select("a:contains(Download)[href*=download/standard]").attr("href"))).select("a:contains(click here)[href*=.exe]").attr("href");
		} catch (IOException e) {
			return null;
		}
	}
