...
public class HoneyviewRemoteRetriever implements RemoteRetriever, BaseUrlRemoteRetriever {
	...
	public String retrieveWin32UrlEn(Document doc) throws IOException {
		return HttpUtils.getRedirectionUrl(ParsingUtils.buildUrl(ROOT_DOWNLOAD_WEBSITE, RemoteServiceImpl.retrieveHtmlDocumentAgentMozilla(ParsingUtils.buildUrl(ROOT_DOWNLOAD_WEBSITE, doc.select("a:contains(Download Honeyview)").attr("href"))).select("li:contains(Server)").first().select("a").attr("href")));
	}
